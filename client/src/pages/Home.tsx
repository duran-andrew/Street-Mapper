...
export default function Home() {
  ...
  const [targetSegment, setTargetSegment] = useState<StreetSegment | null>(null);
  ...
  const loadArea = async (lat: number, lng: number) => {
    ...
    try {
      ...
      toast({
        ...
        description: "Could not fetch street data. Try checking your connection.",
      });
    } finally {
      setLoadingOSM(false);
    }
  ...
  useEffect(() => {
    ...
    if (lastPositionRef.current) {
      const from = turf.point([lastPositionRef.current[1], lastPositionRef.current[0]]);
  ...
  const visitedSegments = useMemo(() => segments.filter(s => s.visited), [segments]);
  const unvisitedSegments = useMemo(() => segments.filter(s => !s.visited), [segments]);
      ...
      >
        ...
        {isActive && targetSegment && coords && (
          <Polyline 
            positions={[
              [coords.lat, coords.lng],
              [targetSegment.geometry.coordinates[0][1], targetSegment.geometry.coordinates[0][0]]
            ]}
            pathOptions={{ 
              color: '#3b82f6', 
              weight: 2, 
      ...
      <div className="absolute inset-0 z-[500] pointer-events-none p-4 md:p-6 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start pointer-events-auto">
          <div className="glass-panel p-2 rounded-full shadow-lg">
             <div className="flex items-center gap-2 px-2">
               ...
               <span className="font-bold text-sm tracking-tight text-foreground">StreetConqueror</span>
             </div>
          </div>
        ...
        {geoError && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-destructive text-destructive-foreground px-6 py-4 rounded-2xl flex flex-col items-center gap-2 shadow-2xl pointer-events-auto text-center max-w-xs">
            ...
            <span className="text-xs">Please enable location services to use this app. ({geoError.message})</span>
          </div>
        )}
